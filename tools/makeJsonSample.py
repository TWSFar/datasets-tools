...
class coco(object):
    ...
    def _load_pascal_annotation(self, index):
        ...
        num_objs = len(target)
        if (self.m + 2) % 425 == 0 and self.n < 6:
            images_js.append(self.coco.imgs[index])
            annotations_js.extend(target)
            self.n += 1
        self.m += 1
...
if __name__ == "__main__":
    ...
    import json
    ann_js = {}
    ann_js['images'] = images_js
    ann_js['categories'] = imdb.coco.dataset['categories']
    ann_js['annotations'] = annotations_js
    json_name = 'train6.json'
